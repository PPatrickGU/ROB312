...
threshold_resampling = 0.5; % seuil de reehantillonnage (theta_eff)
...
for tk = 2:(T/dt)
    ...
    % Reupération de la mesure reelle
    Y = Y_sim(:,tk);
    ...
    % validite de la mesure reelle (a completer pour la gestion des fréquences et des trous de mesures)
    ...
    if is_measurementValid
        ...
        for i=1:N
            ...
            inno = Y - Y_hat;
            likelihood = exp(-inno.^2/(2*Rf)); % correction des poids
            wp(i) = likelihood*wp(i);  
        end
        wp = wp/sum(wp); % normalisation des poids (la somme doit etre égale ?1)
