...
Modified : Goran Frehse, David Filliat, Dajing Gu (in help of Iad)
"""
...
M_DIST_TH = 9  # Threshold of Mahalanobis distance for data association. #9
...
Py_sim = (1 * np.diag([np.deg2rad(5)])) ** 2 # modification
...
def observation(xTrue, xd, uTrue, Landmarks): # modification
    """
    ...
    for i in range(len(Landmarks[:, 0])):
        ...
        if d <= MAX_RANGE:
            angle_n = angle + np.random.randn() * Py_sim[0, 0] ** 0.5  # add noise
...
def jacob_h(q, delta, x, i):
    ...
    """
    G = np.array([[delta[1, 0], -delta[0, 0], -q,  -delta[1, 0], delta[0, 0]]]) # modification
    G = G / q
    nLM = calc_n_lm(x)
...
# ray initialisation parameters
Ng = 3
alpha = 0.3
beta = 3
dmin = 0.5

def calc_innovation(xEst, PEst, y, LMid): # modification
    """
    ...
    yp = np.array([[pi_2_pi(y_angle)]])  # modification

    # compute innovation, i.e. diff with real observation
    innov = (y - yp).T
    innov[0] = pi_2_pi(innov[0])
...
def ekf_slam(xEst, PEst, u, y):  # modification
    """
    ...
    for iy in range(len(y[:, 0])):  # for each observation
        ...
        if KNOWN_DATA_ASSOCIATION:
            try:
                ...
                min_id = trueLandmarkId.index(y[iy, 1])
            except ValueError:
                min_id = nLM
                # trueLandmarkId.append(y[iy, 2])
                trueLandmarkId.append(y[iy, 1])
        else:
            min_id = search_correspond_landmark_id(xEst, PEst, y[iy, 0:1])
        ...
        if min_id == nLM:
            ...
            for iRayInit in range(Ng):
                # create hypo new amer range and range noise
                ...
                PEst = np.vstack((np.hstack((PEst, rightPart)),
                                  ...
                                  Jr @ PEst[0:3, 0:3] @ Jr.T + Jy @ Py_hypo @ Jy.T))))

        else:
            # Perform Kalman update
            # innov, S, H = calc_innovation(xEst, PEst, y[iy, 0:2], min_id)
            innov, S, H = calc_innovation(xEst, PEst, y[iy, 0:1], min_id)
